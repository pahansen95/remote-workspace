...
if [[ "${args[0],,}" == "up" ]]; then
  ...
  if [[ ! -e "${CONTEXT}/.ssh/server_id_ed25519" ]]; then
    mkdir -p "${CONTEXT}/.ssh"
    ssh-keygen -t ed25519 -C "remote-workspace" -N '' -f "${CONTEXT}/.ssh/server_id_ed25519"
  fi
  ...
  jq -n -c \
    ...
    --arg serverPrivKey "$(< "${CONTEXT}/.ssh/server_id_ed25519")" \
    --arg serverPubKey "$(< "${CONTEXT}/.ssh/server_id_ed25519.pub")" \
    '{
      workspace: {
        ssh: {
          keys: {
  ...
  info "Autostarting VsCode Not Implemented yet"
  # code \
